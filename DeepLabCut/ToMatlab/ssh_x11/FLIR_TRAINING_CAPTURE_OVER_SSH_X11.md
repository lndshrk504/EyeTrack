...
conda environment, changes into `DeepLabCut/Tests`, and runs:
...
cd ~/Desktop/BehaviorBox/EyeTrack/DeepLabCut/Tests
python capture_flir_training_frames.py \
  --seconds 60 \
  --save-every 10 \
  --no-preview
...
```bash
./open_training_capture_over_ssh.sh --host wbs@10.55.0.1 -- --scale 0.25 --auto-contrast
```
