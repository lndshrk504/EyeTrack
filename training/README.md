...
```bash
python training/train_dlc_eye_model.py init-project \
...
```bash
python training/train_dlc_eye_model.py extract-frames \
...
```bash
python training/train_dlc_eye_model.py create-dataset \
  --config /data/dlc_projects/PupilTracking-YourName-YYYY-MM-DD/config.yaml
```
...
```bash
python training/train_dlc_eye_model.py train \
  --config /data/dlc_projects/PupilTracking-YourName-YYYY-MM-DD/config.yaml \
  --maxiters 200000 \
  --saveiters 10000
```
...
```bash
python training/train_dlc_eye_model.py evaluate \
  --config /data/dlc_projects/PupilTracking-YourName-YYYY-MM-DD/config.yaml \
  --plotting
```
...
```bash
python training/train_dlc_eye_model.py export \
  --config /data/dlc_projects/PupilTracking-YourName-YYYY-MM-DD/config.yaml \
  --overwrite
```
...
```text
EyeTrack/models/<your-exported-model-folder>/
```

You can then run inference with:

```bash
python DeepLabCut/Tests/smoke_dlc_flir_inference.py --model-path EyeTrack/models/<your-exported-model-folder>
```

## Notes

- The default keypoint order matches the existing YangLab pupil-8 naming convention used in this repository.
- If you change keypoints/order, also update runtime `--kp-*` arguments in `DeepLabCut/ToMatlab/dlc_eye_streamer.py` usage.
